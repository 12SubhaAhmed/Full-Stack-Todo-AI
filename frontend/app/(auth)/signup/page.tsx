...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirection to login page is handled within the register function in AuthContext
    } catch (err: any) {
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-950 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-gray-900/80 backdrop-blur-md rounded-2xl shadow-2xl p-8 space-y-8 border border-purple-500/30 relative z-10">
        <div className="text-center">
          <h1 className="text-4xl font-black bg-clip-text text-transparent bg-linear-to-r from-purple-400 to-pink-500">
            JOIN US
          </h1>
          <p className="mt-2 text-sm text-gray-400 font-medium tracking-wide">Create your account to start organizing</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label htmlFor="name" className="block text-sm font-bold text-gray-300 mb-2 ml-1">
              FULL NAME
            </label>
            <div className="mt-1">
              <input
